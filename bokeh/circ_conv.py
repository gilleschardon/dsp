...
def generate_layout():
    ...
    code = """
        ...
        for (var k = 0 ; k < nx.length ; k++)
        {
             ...
             {
                     z[(k+l)%nx.length] = z[(k+l)%nx.length] + x[k] * y[l]
            }
        }
    ...
    plot_x = figure(x_range=(-marge, L+marge), y_range=(-2.1,2.1) , plot_width=800, plot_height=300)
    ...
    circx = plot_x.circle('n', 'x', source=source_x, line_width=0, line_alpha=1, size=10, legend_label="x", nonselection_fill_alpha=1, color="green")
    ...
    plot_y = figure(x_range=(-marge, L+marge), y_range=(-2.1,2.1) , plot_width=800, plot_height=300)
    ...
    circy = plot_y.circle('n', 'y', source=source_y, line_width=0, line_alpha=1, size=10, legend_label="y", color='red', nonselection_fill_alpha=1)
    ...
    plot_z = figure(x_range=(-marge, L+marge), y_range=(-2.1,2.1) , plot_width=800, plot_height=300)
    #circz0 = plot_z.circle(np.hstack([np.arange(-marge,0), np.arange(2*L-1, 2*L+marge+1)]), 0,  line_width=0, alpha=alpha_out, size=10)
    
    
    plot_z.circle('n', 'z', source=source_z, line_width=0, line_alpha=1, size=10, legend_label="x⍟y")
    plot_z.segment('n', 0, 'n', 'z', source=source_z, line_width=1, line_alpha=1)
    plot_z.xaxis.fixed_location = 0;
    
    pxz = plot_z.circle('nz', 'xz', source=source_x, line_width=0, alpha=0.2, size=20, color="green")
    cxz = plot_z.segment('nz', 0, 'nz', 'xz', source=source_x, line_width=1, line_alpha=0.5, color="green")
    
    
    pyz = plot_z.circle('nz', 'yz', source=source_y, line_width=0, alpha=0.2, size=20, color='red')
